...
export const ArrowUpAnimation = forwardRef((props: Props, ref) => {
  const {
    attackId,
    animationDuration,
    containerNode,
  ...
  const distance =
    targetBounds && Math.round(targetBounds.x - characterBounds.x);
  const heightDiff =
    targetBounds && Math.round(targetBounds.y - characterBounds.y);

  const path =
    heightDiff !== undefined &&
    distance !== undefined &&
    `M0,0 C${distance / 2},${-200 + heightDiff} , ${distance / 2},${
      -200 + heightDiff
    }, ${distance}, ${heightDiff}`;
...
ArrowUpAnimation.displayName = 'ArrowUpAnimation';
